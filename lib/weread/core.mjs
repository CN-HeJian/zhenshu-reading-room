...
/**
 * @param {{ bookId: string | number, chapterUid?: string | number | null, range?: string | null }} input
 * @returns {string | null}
 */
export function buildWeReadLink({ bookId, chapterUid = null, range = null }) {
  if (!bookId) return null;
  const parsedRange = parseRange(range);
  if (chapterUid && parsedRange) {
    ...
    return `weread://bestbookmark?${query.toString()}`;
  }
  if (chapterUid) {
    const query = new URLSearchParams({ bId: String(bookId), chapterUid: String(chapterUid) });
    return `weread://reading?${query.toString()}`;
  }
  return `weread://reading?bId=${encodeURIComponent(String(bookId))}`;
}
