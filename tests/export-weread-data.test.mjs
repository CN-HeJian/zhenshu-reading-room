...
test("exports complete current WeRead data for GitHub Pages", async () => {
  ...
  const responses = new Map([
    ["/shelf/sync", {
      books: [{ bookId: "b1", deepLink: "weread://official/book/b1", title: "山月记", author: "中岛敦", cover: "https://img.test/cover.jpg", readUpdateTime: 10 }],
    ...
    ["/book/bookmarklist", {
      ...
      updated: [{ bookmarkId: "h1", bookId: "b1", chapterUid: 7, markText: "人生是一场误读。", createTime: 1_700_000_000, range: "1-9", deepLink: "weread://official/highlight/h1" }],
    }],
    ["/review/list/mine", {
      hasMore: 0,
      reviews: [{ review: { reviewId: "r1", bookId: "b1", content: "这一段留住。", abstract: "被月光照亮的原文。", range: "10-20", chapterUid: 7, chapterIdx: 3, createTime: 1_700_000_100, chapterName: "文字祈祷", deepLink: "weread://official/review/r1" } }],
    }],
  ]);
  ...
  assert.equal(data.books[0].link, "weread://official/book/b1");
  assert.equal(data.notes.length, 2);
  assert.equal(data.notes[0].quote, "被月光照亮的原文。");
  assert.equal(data.notes[0].chapterIdx, 3);
  assert.equal(data.notes[0].link, "weread://official/review/r1");
  assert.equal(data.notes[1].link, "weread://official/highlight/h1");
  ...
  assert.equal(bodies.every((body) => body.skill_version === "1.0.4"), true);
...
test("does not invent deep links when the API omits them", async () => {
  const responses = new Map([
    ["/shelf/sync", { books: [{ bookId: "b1", title: "无链接之书" }], albums: [], mp: null }],
    ["/book/getprogress", { book: { progress: 1 } }],
    ["/user/notebooks", { hasMore: 0, books: [] }],
  ]);
  for (const mode of ["weekly", "monthly", "annually", "overall"]) {
    responses.set(`/readdata/detail:${mode}`, { totalReadTime: 0 });
  }
  ...
  assert.equal(data.books[0].link, null);
});
